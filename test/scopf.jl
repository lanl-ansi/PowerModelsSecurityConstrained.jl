...
@testset "scopf cuts dc soft 2, with cuts - $(i)" for (i,network) in enumerate(networks)
    ...
    cuts = check_contingencies_branch_power_pm(network, total_cut_limit=2, gen_flow_cuts=[], branch_flow_cuts=[])
...
@testset "scopf cuts dc soft 2, with cuts - $(i)" for (i,network) in enumerate(networks)
    ...
    cuts = check_contingencies_branch_power_bpv_pm(network, total_cut_limit=2, gen_flow_cuts=[], branch_flow_cuts=[])
