...
@testset "scopf cuts dc soft bpv, with cuts - $(i)" for (i,network) in enumerate(c1_networks)
    ...
    cuts = check_c1_contingencies_branch_power_bpv(network, total_cut_limit=2, gen_flow_cuts=[], branch_flow_cuts=[])
...
@testset "scopf cuts ac soft bpv, with cuts - $(i)" for (i,network) in enumerate(c1_networks)
    ...
    result = run_c1_scopf_cuts_soft_bpv(network, ACPPowerModel, nlp_solver)
