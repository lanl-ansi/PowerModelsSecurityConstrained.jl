...
@everywhere function solution2_solver(process_data)
    ...
    for cont in contingencies
        if cont.type == "gen"
            ...
            time_start = time()
            result = run_fixpoint_pf_bqv_native!(network_tmp, pg_lost, nlp_solver, iteration_limit=10)
        ...
        elseif cont.type == "branch"
            ...
            time_start = time()
            result = run_fixpoint_pf_bqv_native!(network_tmp, 0.0, nlp_solver, iteration_limit=10)
            debug(LOGGER, "second-stage contingency solve time: $(time() - time_start)")
