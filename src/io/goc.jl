...
"""
function parse_c1_files(ini_file; scenario_id="")
...
function parse_c1_files(con_file, inl_file, raw_file, rop_file; ini_file="", scenario_id="none")
    ...
    contingencies = parse_c1_con_file(files["con"])
...
const _c1_branch_contigency_structure = [
    1 => "OPEN",
    2 => "BRANCH",
    3 => "FROM",
    4 => "BUS",
...
const _c1_generator_contigency_structure = [
    1 => "REMOVE",
    2 => "UNIT",
    #3 => "ID",
    4 => "FROM",
...
function parse_c1_con_file(file::String)
    open(file) do io
        return parse_c1_con_file(io)
    end
end

function parse_c1_con_file(io::IO)
    con_lists = []
    ...
    while token_idx <= length(tokens)
        ...
        elseif token == "CONTINGENCY"
            ...
            if token == "OPEN" # branch contingency case
                ...
                #if !all(branch_tokens[idx] == val for (idx, val) in _c1_branch_contigency_structure) && !all(branch_tokens[idx] == val for (idx, val) in _branch_contigency_structure_alt)
                if any(branch_tokens[idx] != val for (idx, val) in _c1_branch_contigency_structure)
            ...
            elseif token == "REMOVE"
                ...
                if any(generator_tokens[idx] != val for (idx, val) in _c1_generator_contigency_structure)
