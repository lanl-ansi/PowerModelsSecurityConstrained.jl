...
function write_solution1(network; output_dir="", solution_file="solution1.txt")
...
function write_solution2(network, contingencies; output_dir="", solution_file="solution2.txt")
    if length(output_dir) > 0
        solution_path = joinpath(output_dir, solution_file)
    else
        solution_path = solution_file
    ...
    open(solution_path, "w") do sol2
        ...
        for cont_solution in contingencies
            write_solution2_contingency(sol2, network, cont_solution)
        end
    end
...
function write_solution2_contingency(io::IO, network, contingency_solution)
    base_mva = network["baseMVA"]
