...
function constraint_c1_voltage_magnitude_link(pm::_PM.AbstractPowerModel, i::Int; nw_1::Int=nw_id_default, nw_2::Int=nw_id_default)
    constraint_c1_voltage_magnitude_link(pm, nw_1, nw_2, i)
end


"links the voltage voltage_magnitude of two networks together"
function constraint_c1_gen_power_real_link(pm::_PM.AbstractPowerModel, i::Int; nw_1::Int=nw_id_default, nw_2::Int=nw_id_default)
    constraint_c1_gen_power_real_link(pm, nw_1, nw_2, i)
end

"links the generator power of two networks together, with a linear response function"
function constraint_c1_gen_power_real_response(pm::_PM.AbstractPowerModel, i::Int; nw_1::Int=nw_id_default, nw_2::Int=nw_id_default)
    gen = ref(pm, nw_2, :gen, i)
    constraint_c1_gen_power_real_response(pm, nw_1, nw_2, i, gen["alpha"])
end


""
function constraint_c1_thermal_limit_from_soft(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
    ...
    if haskey(branch, "rate_a")
        constraint_c1_thermal_limit_from_soft(pm, nw, f_idx, branch["rate_a"])
    end
end


""
function constraint_c1_thermal_limit_to_soft(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    t_idx = (i, t_bus, f_bus)
    ...
    if haskey(branch, "rate_a")
        constraint_c1_thermal_limit_to_soft(pm, nw, t_idx, branch["rate_a"])
    end
end


""
function constraint_c1_gen_power_real_deviation(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    gen = ref(pm, nw, :gen, i)

    constraint_c1_gen_power_real_deviation(pm, nw, i, gen["pg"])
end


""
function constraint_c1_branch_contingency_ptdf_thermal_limit_from(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    cut = ref(pm, :branch_flow_cuts, i)
    branch = ref(pm, nw, :branch, cut.branch_id)

    if haskey(branch, "rate_c")
        constraint_c1_branch_contingency_ptdf_thermal_limit_from(pm, nw, i, cut.bus_injection, branch["rate_c"])
    end
end

""
function constraint_c1_branch_contingency_ptdf_thermal_limit_to(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
    cut = ref(pm, :branch_flow_cuts, i)
    branch = ref(pm, nw, :branch, cut.branch_id)

    if haskey(branch, "rate_c")
        constraint_c1_branch_contingency_ptdf_thermal_limit_to(pm, nw, i, cut.bus_injection, branch["rate_c"])
    end
end
...
# ""
# function constraint_c1_gen_contingency_ptdf_thermal_limit_from_soft(pm::_PM.AbstractPowerModel, i::Int; nw::Int=nw_id_default)
